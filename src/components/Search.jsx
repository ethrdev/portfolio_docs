                ...
                queries: [
                  {
                    ...
                    params: {
                      hitsPerPage: 5,
                      highlightPreTag:
                        '<mark class="underline bg-transparent text-teal-500">',
...
function SearchResult({ result, resultIndex, autocomplete, collection }) {
  ...
  return (
    <li
      className={clsx(
        'group block cursor-default px-4 py-3  aria-selected:bg-neutral-800/50',
        resultIndex > 0 && 'border-t  border-neutral-800'
      )}
    ...
    >
      <div
        id={`${id}-title`}
        aria-hidden="true"
        className="text-sm font-medium text-white group-aria-selected:text-teal-500"
      ...
      {hierarchyHtml.length > 0 && (
        <div
          id={`${id}-hierarchy`}
          aria-hidden="true"
          className="mt-1 truncate whitespace-nowrap text-2xs text-[#757575]"
        >
  ...
  if (collection.items.length === 0) {
    return (
      <div className="p-6 text-center">
        <NoResultsIcon className="w-5 h-5 mx-auto  stroke-[#757575]" />
        <p className="mt-2 text-xs  text-[#757575]">
          Nothing found for{' '}
          <strong className="font-semibold text-white break-words">
            &lsquo;{query}&rsquo;
          </strong>
          . Please try again.
        </p>
...
const SearchInput = forwardRef(function SearchInput(
  ...
  return (
    <div className="relative flex h-12 group">
      <SearchIcon className="absolute top-0 w-5 h-full pointer-events-none left-3 stroke-[#757575]" />
      <input
        ref={inputRef}
        className={clsx(
          'flex-auto appearance-none bg-transparent pl-10  outline-none placeholder:text-[#757575] focus:w-full focus:flex-none text-white sm:text-sm [&::-webkit-search-cancel-button]:hidden [&::-webkit-search-decoration]:hidden [&::-webkit-search-results-button]:hidden [&::-webkit-search-results-decoration]:hidden',
      ...
      {autocompleteState.status === 'stalled' && (
        <div className="absolute inset-y-0 flex items-center right-3">
          <LoadingIcon className="w-5 h-5 text-teal-400 animate-spin stroke-neutral-800" />
        </div>
      )}
    </div>
  )
...
function SearchButton(props) {
  ...
  return (
    <>
      <button
        type="button"
        className="hidden h-8 w-full items-center gap-2 rounded-md  pl-2 pr-3 text-sm  ring-1  transition  bg-white/5 text-[#757575] ring-inset ring-white/10 hover:ring-white/20 lg:flex focus:[&:not(:focus-visible)]:outline-none"
        {...props}
      >
        <SearchIcon className="w-5 h-5 stroke-current" />
        Find something...
        <kbd className="ml-auto text-2xs  text-[#757575]">
      ...
      <button
        type="button"
        className="flex h-6 w-6 items-center justify-center rounded transition  hover:bg-white/5 lg:hidden focus:[&:not(:focus-visible)]:outline-none"
    ...
    >
      ...
      >
        ...
        <div className="fixed inset-0 overflow-y-auto px-4 py-4 sm:py-20 sm:px-6 md:py-32 lg:px-8 lg:py-[15vh]">
          ...
          >
            <Dialog.Panel className="mx-auto overflow-hidden rounded shadow-xl ring-1 bg-neutral-900 ring-neutral-800 sm:max-w-xl">
              <div {...autocomplete.getRootProps({})}>
                ...
                >
                  ...
                  <div
                    ref={panelRef}
                    className="border-t   empty:hidden border-[#fefefe]/5 bg-white/2.5"
                  ...
                  >
                    {autocompleteState.isOpen && (
                      <>
                        ...
                        />
                        <p className="flex items-center justify-end gap-2 px-4 py-2 text-xs border-t  border-neutral-800 text-[#757575]">
                          Search by{' '}
                          <AlgoliaLogo className="h-4  fill-[#757575]" />
                        </p>
                      </>
                    )}
                  </div>
...
export function Search() {
  ...
  return (
    <div className="hidden lg:block lg:max-w-md lg:flex-auto">
      <button
        type="button"
        className="hidden h-8 w-full items-center gap-2 rounded  pl-2 pr-3 text-sm  ring-1  transition  bg-white/5 text-[#757575] ring-inset ring-white/10 hover:ring-white/20 lg:flex focus:[&:not(:focus-visible)]:outline-none"
        {...buttonProps}
      >
        <SearchIcon className="w-5 h-5 stroke-current" />
        Find something...
