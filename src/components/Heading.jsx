    ...
    >
      {inView && (
        <div className="absolute mt-1 ml-[calc(-1*var(--width))] hidden w-[var(--width)] opacity-0 transition [--width:calc(2.625rem+0.5px+50%-min(50%,calc(theme(maxWidth.lg)+theme(spacing.8))))] group-hover:opacity-100 group-focus:opacity-100 md:block lg:z-50 2xl:[--width:theme(spacing.10)]">
          <div className="group/anchor block h-5 w-5 rounded-lg ring-1 ring-inset transition  bg-neutral-800 ring-neutral-700 hover:bg-neutral-700 hover:ring-[#757575]">
            <AnchorIcon className="h-5 w-5  transition stroke-[#757575] group-hover/anchor:stroke-white" />
          </div>
        </div>
      )}
      {children}
