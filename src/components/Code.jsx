  ...
  return (
    ...
    >
      <span
        aria-hidden={copied}
        className={clsx(
          'pointer-events-none flex items-center gap-0.5 text-neutral-800 transition duration-300',
          copied && '-translate-y-1.5 opacity-0'
        )}
      >
        <ClipboardIcon className="w-5 h-5 transition-colors fill-[#757575]/20 stroke-[#757575] group-hover/button:stroke-[#757575]" />
        Copy
      </span>
      <span
        aria-hidden={!copied}
...
function CodePanelHeader({ tag, label }) {
  ...
  return (
    <div className="flex items-center gap-2 px-4 h-9 border-y border-t-transparent border-b-white/5 bg-white/1">
      {tag && (
      ...
      {tag && label && (
        <span className="h-0.5 w-0.5 rounded-full bg-[#757575]" />
      )}
      {label && (
        <span className="font-mono text-xs text-[#757575]">{label}</span>
      )}
    </div>
  )
}
...
function CodeGroupHeader({ title, children, selectedIndex }) {
  ...
  return (
    <div className="flex min-h-[calc(theme(spacing.12)+1px)] flex-wrap items-start gap-x-4 border-b  px-4 border-neutral-800 bg-transparent">
      {title && (
            ...
            <Tab
              className={clsx(
                ...
                childIndex === selectedIndex
                  ...
                  : 'border-transparent text-[#757575] hover:text-[#757575]'
              )}
            >
...
export function CodeGroup({ children, title, ...props }) {
  ...
  return (
    <CodeGroupContext.Provider value={true}>
      <Container
        {...containerProps}
        className="my-6 overflow-hidden rounded-md shadow-md not-prose ring-1 ring-white/10"
      >
